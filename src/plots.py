...
def plot_freight_value_weight_relationship(df: DataFrame):
    """
    Scatterplot of product weight (x) vs. freight value (y).
    ...
    df : DataFrame
        ...
        - 'freight_value' : numeric (currency units)
    """

    # ------------------------------------------------------------------
    # Figure / axis – set style for consistency
    # ------------------------------------------------------------------
    ...
    # Optional trend line – helps the viewer see correlation
    sns.regplot(
        data=df,
        x="product_weight_g",
        y="freight_value",
    ...
    ax.set_title("Freight Value vs. Product Weight", fontsize=14, pad=12)
...
def plot_order_amount_per_day_with_holidays(df):
    """
    Plot the number of orders per day in 2017 and highlight Brazilian public
    holidays with a vertical red line.

    Parameters
    ----------
    df : pandas.DataFrame
        Must contain three columns:
        - 'date'         (datetime.date or datetime64[ns])
        - 'order_count'  (int)
        - 'holiday'      (bool)  True if the date is a public holiday
    """
    # ------------------------------------------------------------------ #
    # Basic figure / axis
    # ------------------------------------------------------------------ #
    ...
    # ------------------------------------------------------------------ #
    # Plot the daily order count line
    # ------------------------------------------------------------------ #
    ax.plot(df["date"], df["order_count"], color="steelblue", lw=1.3, label="Orders")

    # ------------------------------------------------------------------ #
    # Add vertical lines for holidays
    # ------------------------------------------------------------------ #
    holiday_dates = df.loc[df["holiday"], "date"]
    for hd in holiday_dates:
        ax.axvline(hd, color="firebrick", alpha=0.35, lw=0.8)

    # ------------------------------------------------------------------ #
    # Cosmetics
    # ------------------------------------------------------------------ #
    ax.set_title("Number of Orders per Day in 2017 – Holidays Highlighted", fontsize=14)
    ax.set_xlabel("Date")
